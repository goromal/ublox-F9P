...
class Spoofer(object):
    def __init__(self):
        ...
        self.velsub = rospy.Subscriber("mocap_vel", TwistStamped, self.velCallback, queue_size=1)

        if self.frame_id == 'rover':
            self.otherSub = rospy.Subscriber("base_mocap", PoseStamped,
    ...
    def otherCallback(self, msg):
        # Mocap frame is NWU, not worrying about orientation thankfully
    ...
    def velCallback(self, msg):
        # Mocap position differentiation all happens in the NWU frame, I believe
        vN = msg.twist.linear.x
        vE = -msg.twist.linear.y
        vD = -msg.twist.linear.z
        self.velNED = [vN, vE, vD]
